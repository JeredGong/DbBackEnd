...
pub async fn Statistics(
    ...
    .map_err(|err| {
        println!("Database error: {:?}", err);
        actix_web::error::ErrorInternalServerError(format!(
            "Failed to get count of upload documents.\nDatabase error: {}",
            err
        ))
    })?;
    ...
    let borrowBook = sqlx::query!("SELECT COUNT(id) AS count FROM recs")
        ...
        .map_err(|err| {
            println!("Database error: {:?}", err);
            actix_web::error::ErrorInternalServerError(format!(
                "Failed to get books borrowed count.\nDatabase error: {}",
                err
            ))
        })?;
    ...
    let dnloadDocs = sqlx::query!("SELECT SUM(download_count) AS count FROM docs")
        ...
        .map_err(|err| {
            println!("Database error: {:?}", err);
            actix_web::error::ErrorInternalServerError(format!(
                "Failed to get documents downloaded count.\nDatabase error: {}",
                err
            ))
        })?;
    ...
    let statisticsResponse = StatisticsResponse {
        ...
        borrowBook: if userID == 0 {
            0
        } else {
            borrowBook.count.unwrap_or_default()
        },
        dnloadDocs: if userID == 0 {
            0
        } else {
            dnloadDocs.count.unwrap_or_default()
        },
    };
